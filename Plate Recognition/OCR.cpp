...
void OCR::train(Mat X, Mat classes, int nlayers) {
    Mat layers(1, 3, CV_32SC1);
    layers.at<int>(0) = X.cols;
    ...
    Mat trainClasses;
    trainClasses.create(X.rows, numCharacters, CV_32FC1);
    ...
    Mat weights(1, X.rows, CV_32FC1, Scalar::all(1));

    //Learn classifier
    //cout <<X.size()<<X.rows << " " << X.cols << endl;
    //cout << trainClasses.size() << endl;
    Ptr<TrainData> trainDatas = TrainData::create(X, ROW_SAMPLE, trainClasses);
